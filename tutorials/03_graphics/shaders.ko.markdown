...
*gl_FragCoord*는 버텍스 쉐이더에서 전달되는 vec2 "출력"으로 교체될 수도 있습니다(혼란스럽죠, 알고 있습니다). 마침내, 프래그먼트 쉐이더를 위한 "출력" 변수가 있군요.
...
이것은 좀 더 재밌을겁니다: 이제 쉐이더를 가지고 움직여볼겁니다. 우선, ofApp.cpp입니다. 비행기를 만들겁니다, 가지고 놀 훌륭한 버텍스들을 가지고요. ofPlanePrimitive가 "가지고 놀 버텍스들"로 완벽하군요. 한번 사용해보죠:
...
코드 내에서 *ofSetColor(colorMix)*를 호출했다는 사실에 주목하세요. 저 부분이 우리가 *사용할 수 있는* 프래그먼트 쉐이더에 실제로 색을 지정해줍니다. "out vec4 color"를 
우리가 원하는 오래된 것들에 적용할 수 있지만, 이번의 경우, 버텍스 쉐이더안에서 할수 있는 재미난 부분이 너무 많기 떄문에 그렇게 하지 않았습니다.

자, 각 버텍스를 위해 버텍스쉐이더에서 한번의 호출을 했기 떄문에, 이것들을 프래그먼트 쉐이더에 넘기기 전에 버텍스들의 위치를 가지고 놀 수 있습니다. *gl_Position*이 버텍스 쉐이더로부터의 산출물이므로, 우리가 어떻게 지정을 하든 버텍스의 위치가 됩니다. 이번의 경우, 약간의 *sin()*웨이브를 사용해서, 우리의 평면에 멋진 웨이브 효과를 줄 수 있습니다:
