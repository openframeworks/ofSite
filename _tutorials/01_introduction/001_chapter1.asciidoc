...
.Code::Blocks note
NOTE: This section focuses on XCode as the example IDE. All the general information provided here also applies to Code::Blocks and can be transferred accordingly. The Code::Blocks project file has the ".workspace" extension and is used instead of ".xcodeproj". Please consult the link:http://www.codeblocks.org/user-manual[Code::Blocks manual] if you have any IDE specific questions.
...
NOTE: First we will [underline]#duplicate# (NOT rename) the entire "emptyExample"
