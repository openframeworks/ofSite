                         ...
                         if (standard)
                         {
                              ...
                              for (var i = 0; i < tipuesearch_in.pages.length; i++)
                              {
                                   ...
                                   for (var f = 0; f < d_w.length; f++)
                                   {
                                       ...
                                       {
                                            if(tipuesearch_in.pages[i].title.match("^" + pat + "$")!==null){       
                                                score += 500;
                                            }else{     
                                        ...
                                        tags = tipuesearch_in.pages[i].tags.split(" ")
                                        foundmatch = false;
                                        type = "";
                                        for(idx in tags){
                                            if(tags[idx].search(pat)!=-1){
                                                if(tags[idx].match("^" + pat + "$")!==null){       
                                                    score += 100;
                                                }else{     
                                                    var m_c = tags[idx].match(pat).length;
                                                    score += (10 * m_c);
                                                }
                                            }
                                            
                                            if(tags[idx]=="module") type = "module";
                                            if(tags[idx]=="class") type = "class";
                                            if(tags[idx]=="function") type = "function";
                                            
                                            distance = levenshtein(d_w[f], tags[idx]);
                                            tags_distance += distance;
                                            if(distance>0){
                                                distance /= Math.max(d_w[f].length, tags[idx].length)
                                                if(distance<0.5){
                                                    if(!foundmatch) score += (1-distance) * 100
                                                    foundmatch = true;
                                                }else{
                                                    score -= distance * 10
                                                }
                                            }else{
                                                if(!foundmatch) score += 100
                                                foundmatch = true;
                                            }
                                        }
                                        
                                        if(foundmatch){
                                            if(type === "module") score+=30;
                                            if(type === "class") score+=20;
                                            if(type === "function") score+=10;
                                        }else{
                                            score -= 20;
                                        }
                                       ...
                                       {
                                            var m_c = tipuesearch_in.pages[i].text.match(pat).length;
                                            if(type === "module") score+=10*m_c;
                                            else if(type === "class") score+=7*m_c;
                                            else if(type === "function") score+=5*m_c;
                                            else score += (3 * m_c);
                                       }else{
                                            foundmatch = false;
                                            words = tipuesearch_in.pages[i].text.replace('<br/>',' ').split(' ')
                                            for(idx in words){
                                                if(levenshtein(d_w[f], words[idx])<2){
                                                    score += 2;
                                                    foundmatch = true;
                                                }
                                            }
                              ...
                              for (var i = 0; i < found.length; i++)
                              {
                                   ...
                                   {                                   
                                        ...
                                        {                                             
                                             ...
                                             out += '<div class="tipue_search_content_debug">Distance: ' + found[i].tags_distance + '</div>';
                                        }
