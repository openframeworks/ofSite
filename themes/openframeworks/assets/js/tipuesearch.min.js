...
(n=f.pages[a].title.match(g).length,d+=100*n);-1!=f.pages[a].text.search(g)&&(n=f.pages[a].text.match(g).length,d+=20*n);h.highlightTerms&&(n=h.highlightEveryTerm?new RegExp("("+e+")","gi"):new RegExp("("+e+")","i"),r=r.replace(n,'<span class="h01">$1</span>'));-1!=f.pages[a].tags.search(g)&&(n=f.pages[a].tags.match(g).length,d+=50*n);-1!=f.pages[a].url.search(g)&&(d+=20);if(0!=d)for(g=0;g<tipuesearch_weight.weight.length;g++)f.pages[a].url==tipuesearch_weight.weight[g].url&&(d+=tipuesearch_weight.weight[g].score);
