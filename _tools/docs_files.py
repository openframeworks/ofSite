...
def list_all_classes(db,groupid,advanced):
    cursor=db.cursor()
    sql='SELECT c.id,c.name FROM docs_class c JOIN docs_files f WHERE c.fileid=f.id and f.groupid=%s and f.advanced=%s and c.advanced=%s and f.visible=1 and c.visible=1 ORDER BY f.sortid, c.sortid'
