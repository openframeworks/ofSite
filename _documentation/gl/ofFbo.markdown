...
Often the ofFbo will contain artifacts from the memory that the graphics card has just allocated for it, so it's good to clear it before starting to draw it:
...
When you call begin() you're telling the framebuffer to store the rendered results of any drawing calls (or shaders for that matter) in the ofFbo:
...
When it's time to draw your ofFbo, you can simply call draw:
...
If you want to pass the ofFbo to say, an ofShader, you do:

~~~~{.cpp}
shader.setUniformTexture("fboTexture", fbo.getTextureReference(0), 0);
~~~~

You can also use the ofFbo::Settings object to create a more customized ofFbo that allows you to set the internal format of the depth and stencil textures, create multiple textures to render to, and use different texture targets, among other things.
...
This method allows you to render the results of a shading pass to all the textures inside the ofFbo. It's  handy if you have many textures inside your ofFbo, for instance, a normals texture, a colored depth texture, a color texture, and you want to have a shader render to all of them at once. It calls glDrawBuffers() internally, which you can learn more about [here](http://www.opengl.org/sdk/docs/man/xhtml/glDrawBuffers.xml).
...
Before you use the ofFbo you need to allocate it. This sets the width, height, and GL type of the ofFbo (i.e. whether it has alpha data or not) and the number of samples for MSAA. MSAA is sort of a big topic.
...
Any drawing that you do after begin() is drawn into the ofFbo rather than the screen. This is how you draw things into your ofFbo instance.
...
This lets you draw the ofFbo using vertices to define the area that the ofFbo will be drawn into, the same way that you would bind an ofTexture. This can be an ofRectangle, ofMesh, or other vertex based drawing technique.
...
This allows you quickly check whether your graphics card supports ofFbo objects.
...
This checks the status of your ofFbo object.
...
The attachment point is the index of the texture that you're going to be referring to within the ofFbo. By default this should just be GL_DEPTH_STENCIL but if you know what you're doing and don't want a stencil buffer you can use GL_DEPTH_ATTACHMENT or vice versa, GL_STENCIL_ATTACHMENT.
...
This creates a texture of the specified format and attaches it to the ofFbo at the index specified. Most of this can be handled for you by using the ofFbo::Settings object. RenderBuffers are slightly different than textures, more info can be found on the [OpenGL Wiki](http://www.opengl.org/wiki/Renderbuffer_Objects).
...
This creates a texture of the specified format and attaches it to the ofFbo at the index specified. Most of this can be handled for you by using the ofFbo::Settings object.
...
Destroys the ofFbo and releases all textures and render-buffers created.
...
This allows you draw everything that's in your ofFbo to the screen using its default height and width at the x, y indicated.
...
This allows you draw everything that's in your ofFbo to the screen using any height and width. Any stretching is up to you to handle appropriately.
...
Any drawing that you do after end() is drawn into the ofFbo rather than the screen. This is how you stop drawing things into your ofFbo instance.
...
This gives you the OpenGL id of the depth Texture that the ofFbo contains. The depthBuffer will be created by default or if you pass a Setting object with depthStencilAsTexture = true.
...
This returns the GLuint of ofFbo for advanced actions, if you're interested in doing something with the ofFbo id directly.
...
This returns the height of the ofFbo. This is just like height of a texture: it sets how many pixels wide the allocated memory on the graphics card is.
...
This returns the number of textures that the ofFbo contains.
...
This gives you the OpenGL id of the stencil RenderBuffer that the ofFbo contains. The depthBuffer will only be created if you pass a Setting object with depthStencilAsTexture = false and settings.useStencil = true.
...
This gives you access to the ofTexture contained w/in the ofFbo. This returns the texture index returned by setDefaultTextureIndex() if you've set it there.
...
This gives you access to a particular ofTexture contained by the ofFbo if there are more than 1 contained w/in the ofFbo. By default you'll have a color texture and a depth texture, though you can configure your ofFbo to have more.
...
This returns the width of the ofFbo that was set when it was allocated. This is just like width of a texture: it sets how many pixels wide the allocated memory on the graphics card is.
...
Returns whether your ofFbo is allocated or not.
...
Copies all data from the mom ofFbo
...
This overloaded operator allows you to set one ofFbo from another using the = operator. Very convenient.
...
This allows you to get the pixels from an ofFbo and store it in an ofPixels instance. The attachmentPoint parameter allows you indicate which of the textures attached to the ofFbo you want to grab.
...
This allows you to get the pixels from an ofFbo and store it in an ofShortPixels instance. The attachmentPoint parameter allows you indicate which of the textures attached to the ofFbo you want to grab. The ofShortPixels instance is useful when you want your image at short ints, or non-floating point values.
...
This allows you to get the pixels from an ofFbo and store it in an ofShortPixels instance. The attachmentPoint parameter allows you indicate which of the textures attached to the ofFbo you want to grab. The ofShortPixels instance is useful when you want your image as floating point values.
...
This allows you set the anchor position of the texture in the ofFbo when you draw it.
...
This allows you set the default texture that your ofFbo will use. If you're using multiple textures, this will return the one that should be draw to, scaled, and positioned.
...
After you bind the ofFbo and draw with it, call ofFbo to stop the ofFbo from being attached to vertices that are created.
